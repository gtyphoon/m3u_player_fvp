...
> 🎯 **对哪种视频流支持最好？** 本软件专门为 **H.265/HEVC 编码**的摄像头流做了优化——这是现在绝大多数监控摄像头默认的输出格式（尤其是 **HEVC-in-FLV** 封装）。对这种流，软件支持**硬件解码**（用手机/电视/电脑自带的视频芯片解码，而不是靠 CPU 硬算），做到**多路同时播放依然清晰流畅、不卡顿不发热**。当然，常见的 H.264 编码流也完全支持。
